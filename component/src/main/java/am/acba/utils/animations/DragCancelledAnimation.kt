package am.acba.utils.animations
...
interface DragCancelledAnimation {
    suspend fun dragCancelled(position: DraggableItemPosition, offset: Offset)
    val position: DraggableItemPosition?
    val offset: Offset
}

class NoDragCancelledAnimation : DragCancelledAnimation {
    override suspend fun dragCancelled(position: DraggableItemPosition, offset: Offset) {}
    override val position: DraggableItemPosition? = null
...
class SpringDragCancelledAnimation(private val stiffness: Float = Spring.StiffnessMediumLow) : DragCancelledAnimation {
    ...
    override var position by mutableStateOf<DraggableItemPosition?>(null)
        private set
