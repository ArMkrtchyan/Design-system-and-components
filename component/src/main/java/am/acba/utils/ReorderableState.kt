...
import am.acba.utils.animations.DragCancelledAnimation
import am.acba.utils.extensions.DraggableItemPosition
import am.acba.utils.extensions.StartDrag
...
abstract class ReorderableState<T>(
    ...
    private val onMove: (fromIndex: DraggableItemPosition, toIndex: DraggableItemPosition) -> (Unit),
    private val canDragOver: ((draggedOver: DraggableItemPosition, dragging: DraggableItemPosition) -> Boolean)?,
    ...
    internal fun onDragCanceled() {
        ...
        if (dragIdx != null) {
            val position = DraggableItemPosition(dragIdx, selected?.itemKey)
    ...
    internal fun onDrag(offsetX: Int, offsetY: Int) {
        ...
        chooseDropItem(
            ...
            if (targetItem.itemIndex == firstVisibleItemIndex || draggingItem.itemIndex == firstVisibleItemIndex) {
                scope.launch {
                    onMove.invoke(
                        DraggableItemPosition(draggingItem.itemIndex, draggingItem.itemKey),
                        DraggableItemPosition(targetItem.itemIndex, targetItem.itemKey)
                    )
    ...
    protected open fun findTargets(x: Int, y: Int, selected: T): List<T> {
        ...
        visibleItemsInfo.fastForEach { item ->
            ...
            if (canDragOver?.invoke(
                    DraggableItemPosition(item.itemIndex, item.itemKey),
                    DraggableItemPosition(selected.itemIndex, selected.itemKey)
                ) != false
            ) {
